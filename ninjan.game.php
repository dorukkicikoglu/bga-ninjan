...
class Ninjan extends Table
{
    ...
    public function actSelectCard($cardID, $autoPlay = false): void
    {
        if(!$autoPlay)
            $this->checkAction('actSelectCard');

        $currentPlayerID = $autoPlay ? $autoPlay : (int) $this->getCurrentPlayerId();
        ...
        $this->notifyPlayer($currentPlayerID, 'notif_cardSelectionConfirmed', '', array('confirmed_selected_card_id' => $cardID));
        
        $this->gamestate->setPlayerNonMultiactive($currentPlayerID, 'displaySelectedCards');
    }

    public function actRevertCardSelection(): void
    {
        $this->gamestate->checkPossibleAction('actRevertCardSelection');

        $currentPlayerID = (int) $this->getCurrentPlayerId();

        self::DbQuery("UPDATE player SET selected_card_id = NULL WHERE player_id = $currentPlayerID");
        ...
        $this->notifyPlayer($currentPlayerID, 'notif_cardSelectionReverted', '', []);
    }
    ...
    public function actTakePile($pileIndex, $autoPlay = false): void
    {
        ...
        if(!in_array($pileIndex, $possiblePiles['pile_indeces']))
...
////////////
    ...
    public function stSelectCard(): void {
        self::DbQuery("UPDATE player SET selected_card_id = NULL, made_card_selection_this_round = 'false'");
        $this->gamestate->setAllPlayersMultiactive();

        if(!$this->tableManager->isLastCards())
            return;

        // auto-play last cards of hands
    ...
    public function stTakePile(): void {
        ...
        if($possiblePiles['reason'] == 'take'){
            if(count($possiblePiles['pile_indeces']) == 1)
                $this->actTakePile($possiblePiles['pile_indeces'][0], true);
            else if($this->globalsManager->getPref('auto_take_best_pile', $activePlayerID) == AUTO_TAKE_BEST_PILE_ON){
                $bestPileScore = -INF;
                $bestPileIndex = false;
                foreach($possiblePiles['pile_indeces'] as $index => $pileIndex){
...
//       }
    ...
    protected function getAllDatas()
    {
        ...
        $myPlayerData = $this->getObjectFromDB( "SELECT sort_cards_by, selected_card_id FROM player WHERE player_id = $current_player_id" );
        if($myPlayerData){
            $result['my_hand'] = $this->handManager->getPlayerHand($current_player_id);
            $result['sort_cards_by'] = $myPlayerData['sort_cards_by'];

            if($this->gamestate->state()['name'] == 'selectCard' && $myPlayerData['selected_card_id'])
