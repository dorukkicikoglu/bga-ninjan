    ...
    return declare("bgagame.ninjan", ebg.core.gamegui, {
        ...
        */
        ...
        onEnteringState: function( stateName, args )
        {
            ...
            switch( stateName )
            {
                case 'selectCard':
                    if(this.myself)
                        this.myself.hand.updateConfirmedSelection(args.args._private.selected_card_id);
                    break;
        ...
        onGameUserPreferenceChanged: function(pref_id, pref_value) {
            switch (pref_id) {
                case 101:
                    this.backgroundHandler.flyingCharactersPreferenceChanged(parseInt(pref_value) == 1);
                break;
            }
        },
                    ...
                    if (mutation.type === 'childList') {
                        mutation.addedNodes.forEach((node) => {
                            if (node.nodeType === 1 && node.tagName.toLowerCase() === 'div' && node.classList.contains('log')){
                                ...
                                dojo.query('.playername', node).forEach((playerName) => { dojo.attr(playerName, 'player-color', this.rgbToHex(dojo.style(playerName, 'color'))); });
                                ...
                                if(dojo.hasClass(node, 'selected-cards-log')){
                                    ...
                                    if(this.isDesktop()){
                                        ...
                                        cardIcons.style.width = 'calc(100% - ' + (10 + this.getPos(dojo.query('.playername', node)[0]).w + this.getPos(dojo.query('.log-arrow', node)[0]).w)  + 'px)';
                                    }
                                }

                                if(this.isDesktop() && dojo.hasClass(node, 'a-game-log')){
        ...
        */
        ...
        notif_cardSelectionConfirmed: function(notif)
        {
            ...
            this.myself.hand.updateConfirmedSelection(notif.args.confirmed_selected_card_id);
        },
        ...
        notif_cardSelectionReverted: function(notif)
        {
            ...
            this.myself.hand.updateConfirmedSelection(false);
        },
